...
const QuickLogCard = ({
  ...
  return (
    <View style={tw.style('mt-4 bg-dark border border-primary/30 rounded-xl overflow-hidden w-64')}>
      ...
      <View style={tw.style('p-4 py-3')}>
        {items.map((item, index) => (
          <View key={index} style={tw.style('mb-3')}>
            <Text style={tw.style('text-white font-montserratSemiBold text-xs mb-1')}>
              {item.category}
            </Text>
            <Text style={tw.style('text-white/60 font-montserrat text-[10px] mb-1')}>
              Estimasi Biaya (Rp)
            </Text>
            <TextInput
              value={costs[index]}
        ...
        <TouchableOpacity
          style={
            [
              ...
              isSaved
                ? tw.style('bg-secondary border border-darkGrey')
                : tw.style('bg-primary'),
            ] as any
          }
          disabled={isSaved}
...
export default function MotoAIScreen({ navigation }: Props) {
  const {
    messages,
    isLoading,
    sendMessage,
  ...
  const { createService, services } = useService();
        ...
        <ScrollView
          ...
          {messages.length === 0 ? (
            <View style={tw.style('items-center justify-center py-20')}>
              ...
              />
              <Text style={tw.style('text-darkGrey font-montserrat text-center px-8 mb-8')}>
              ...
              <View style={tw.style('flex-row flex-wrap justify-center px-2')}>
                {SUGGESTIONS.map((suggestion, index) => (
                  <TouchableOpacity
                    ...
                    <Text style={tw.style('text-white/80 font-montserrat text-xs text-center')}>
                      "{suggestion}"
                    </Text>
                  </TouchableOpacity>
                ))}
            ...
            messages.map((message, index) => (
              <View
                ...
                style={
                  [
                    tw.style('mb-4 max-w-[85%] rounded-2xl p-4'),
                    message.type === 'user'
                      ? tw.style(
                          'bg-primary/20 self-end rounded-tr-sm border border-primary/30',
                        )
                      : tw.style('bg-secondary self-start rounded-tl-sm'),
                  ] as any
                }>
                {message.text ? (
                <Text style={tw.style('text-white font-montserrat text-sm leading-5')}>
                  {message.text}
                </Text>
                ) : null}
                ...
                {message.component === 'MotorcycleStatus' &&
                  activeMotorcycle && (
                    <View style={tw.style('mt-4 bg-dark border border-primary/30 rounded-xl overflow-hidden w-64')}>
                      <View style={tw.style('p-3 bg-secondary/80 border-b border-white/5')}>
                        <View style={tw.style('flex-row items-center mb-2')}>
                          <View style={tw.style('bg-darkGrey rounded-full w-8 h-8 items-center justify-center mr-2')}>
                            <Bike size={14} color={tw.color('white')} />
                          </View>
                          <View>
                            <Text style={tw.style('text-white font-montserratBold text-sm')} numberOfLines={1}>
                        ...
                        <View style={tw.style('flex-row items-center justify-between mt-1')}>
                      ...
                      <View style={tw.style('flex-row items-center')}>
                        <TouchableOpacity
                          ...
                          <Text style={tw.style('text-white font-montserrat ml-1.5 text-xs')}>
                            Update KM
                          </Text>
                        </TouchableOpacity>

                        <TouchableOpacity
                          style={tw.style('flex-1 flex-row items-center justify-center py-2.5 bg-primary/20')}
                          onPress={() =>
                ...
                {message.component === 'QuickLogService' &&
                  ...
                  activeMotorcycle && (
                    <QuickLogCard
                      ...
                      serviceType={(message.data as any).serviceType}
                      items={(message.data as any).items}
                      onSave={(mId, type, totalPrice, sItems) => {
                        createService({
                          motorcycleId: mId,
                          serviceType: type,
                ...
                {message.component === 'Diagnostic' && message.data && (
                  <View style={tw.style('mt-1 w-72')}>
                    <DiagnosticCard
                      symptom={(message.data as any).symptom}
                      question={(message.data as any).question}
                      options={(message.data as any).options}
          ...
          {isLoading && (
            <View style={tw.style('self-start bg-secondary rounded-2xl rounded-tl-sm p-4 mb-4')}>
          ...
          <TouchableOpacity
            ...
            style={
              [
                ...
                (!inputText.trim() || isLoading) && tw.style('opacity-50'),
              ] as any
            }>
